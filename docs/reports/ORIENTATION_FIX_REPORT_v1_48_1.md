...
2. `js/child-profile.js` cuba menjalankan `screen.orientation.lock('landscape')` terus dari page child-select. Ini boleh menyebabkan page profil/dashboard mula rotate sebelum masuk game.
...
- Buang direct `screen.orientation.lock('landscape')` daripada `js/child-profile.js` dan `child-select.js`.
